...
Workflow использует `mastra?.getAgent('kagami-agent')` — продолжит работать, т.к. ключ не изменился. Но `agent.generate()` в workflow не передаёт `maxSteps`, что может быть проблемой для supervisor'а.
...
// After F4, kagami-agent is a supervisor — delegation requires maxSteps >= 10.
